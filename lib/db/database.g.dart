  ...
  Future<sqflite.Database> open(String path, List<Migration> migrations,
    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        ...
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `ServiceTable` (`id` TEXT NOT NULL, `serviceId` TEXT NOT NULL, `captainId` TEXT NOT NULL, `tableNumber` INTEGER NOT NULL, `capacity` INTEGER NOT NULL, `isOccupied` INTEGER NOT NULL, `type` INTEGER NOT NULL, PRIMARY KEY (`id`))');
        await database.execute(
...
class _$BlocDao extends BlocDao {
  ...
  Future<List<ServiceTable>> getTables(String serviceId) async {
    return _queryAdapter.queryList(
        ...
        mapper: (Map<String, Object?> row) => ServiceTable(id: row['id'] as String, serviceId: row['serviceId'] as String, captainId: row['captainId'] as String, tableNumber: row['tableNumber'] as int, capacity: row['capacity'] as int, isOccupied: (row['isOccupied'] as int) != 0, type: row['type'] as int),
        arguments: [serviceId]);
  }
